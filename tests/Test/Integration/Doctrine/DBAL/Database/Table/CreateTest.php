...
class CreateTest extends AbstractIntegrationTestCase
{
    ...
    public function testCreateTableWithPrimaryKeyAndAutoIncrement(): void
    {
        ...
        if ($this->_platform instanceof Firebird3Platform) {
            // Firebird 3+ uses GENERATED BY DEFAULT AS IDENTITY for autoincrement.
            // fbird_last_insert_id() does not yet support IDENTITY columns
            // (php-firebird v10.3.9 limitation). Verify INSERT works correctly
            // and skip the lastInsertId assertion until the extension is fixed.
            foreach ([1, 2] as $id) {
                ...
                if ($lastId === false || $lastId === 0) {
                    self::markTestIncomplete(
                        'fbird_last_insert_id() does not support IDENTITY columns (php-firebird v10.3.9 limitation)'
                    );
                }
                self::assertSame($id, $lastId, 'Incorrect autoincrement value');
            }
        } else {
