...
class FirebirdPlatform extends AbstractPlatform
{
    ...
    public function getSetTransactionIsolationSQL($level): string
    {
        return match ($level) {
            ...
            => 'SET TRANSACTION READ WRITE ISOLATION LEVEL READ UNCOMMITTED RECORD_VERSION',
            TransactionIsolationLevel::READ_COMMITTED
            => 'SET TRANSACTION READ WRITE ISOLATION LEVEL READ COMMITTED RECORD_VERSION',
    ...
    public function getAlterTableSQL(TableDiff $diff): array
    {
        ...
        foreach ($diff->getChangedColumns() as $columnDiff) {
            ...
            if ($columnDiff->hasNotNullChanged()) {
                // Firebird 2.5 requires direct manipulation of system tables for NOT NULL changes
                // See: https://www.firebirdsql.org/file/documentation/chunk/en/refdocs/fblangref25/fblangref25-ddl-tbl.html
                // Note: Firebird 3.0+ overrides this method to use ALTER COLUMN ... SET/DROP NOT NULL
        ...
        return array_values(array_merge($sql, $columnSql));
    }
...
SQL
    ...
    protected function getDateArithmeticIntervalExpression(
        ...
        return 'DATEADD(' . $unit->value . ', ' . $interval . ', ' . $date . ')';
    }
    ...
    protected function getDropSequenceIfExistsPSql(Sequence|string $aSequence, bool $inBlock = false): string
    {
        $result = sprintf(
            ...
            $this->getExecuteStatementPSql(parent::getDropSequenceSQL(
                $aSequence instanceof Sequence ? $aSequence->getQuotedName($this) : $aSequence,
            )),
        );
        if ($inBlock) {
            return $this->getExecuteBlockSql([
    ...
    protected function getBinaryTypeDeclarationSQLSnippet(int|null $length): string
    {
        ...
        return 'CHAR(' . ($length > 0 ? $length : $this->getBinaryMaxLength()) . ')';
    }
